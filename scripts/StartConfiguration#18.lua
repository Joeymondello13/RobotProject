...
StartConfiguration='StartConfiguration#18'
Pioneer_offset='Pioneer_offset#18'
handle_signal='pathHandle2'
replan_signal='replan_path2'
isArrived_signal='getIsArrived2'
collectionHandle=sim.getCollectionHandle('Col3#')
--Robot_name=sim.getObjectName(getPioneerHandle())

function getGoalHandle()
    return sim.getObjectHandle('Pioneer_goal#18')
end
function getGoalConfiguration()
    return sim.getObjectHandle('GoalConfiguration#1')
end
function getPioneerHandle()
    return sim.getObjectHandle('Pioneer_p3dx#18')
end
pathPlanner = function()
    ...
    if sim.getIntegerSignal('robot3Wait') == 0 then
